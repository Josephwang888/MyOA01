				                        ...
				                        <td>
					                        <select class="form-control" onchange="assignRole(this.value,'${user.name}')">
					                        	<c:forEach var="role" items="${allRoles}">
					                        		<option value="${role.id}" <c:if test="${role.name==user.role}">selected</c:if>>${role.name}</option>
					                        	</c:forEach>
					                        </select>
				                        </td>
				                        <td>
				                        	<select class="form-control" onchange="assignRole(this.value,'${user.name}')">
											    <c:forEach var="df" items="${list}">  
						                        	<!-- ${user.managerId} -->
									                 <c:if test="${not empty user.managerId && user.managerId  ne '' }">
									                 	<option value="${df.name}"<c:if test="${user.managerId==df.id}">selected</c:if>>${df.name}</option>
									                 </c:if>
												</c:forEach>
					                        </select>
				                        </td>
				...
				<div class="modal-body">
					<table class="table table-bordered table-striped" width="800px">
						...
						<tr>
						    <td>上级主管</td>
						    <td>
						    	<select  id="selManager" class="form-control" name="manager_id"></select>
						    </td>
						</tr>
